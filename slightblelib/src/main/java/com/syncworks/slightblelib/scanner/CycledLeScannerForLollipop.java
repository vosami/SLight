...
import com.syncworks.slightblelib.service.DetectionTracker;
...
public class CycledLeScannerForLollipop extends CycledLeScanner {
    private static final Object TAG = CycledLeScannerForLollipop.class;
            ...
            if (true) {
                ...
                if (mScanDeferredBefore == false) {
                    if (secsSinceLastDetection > BACKGROUND_L_SCAN_DETECTION_PERIOD_MILLIS) {
                        ...
                        Logger.d(TAG, "This is Android L. Doing a filtered scan for the background.");
                        ...
                        try {
                            ...
                            Logger.w(TAG, "Cannot start scan.  Bluetooth may be turned off.");
                        }

                    } else {
                        Logger.d(TAG, "This is Android L, but we last saw a beacon only %s "
                ...
                if (mBackgroundLScanStartTime > 0l) {
                    ...
                    if (DetectionTracker.getInstance().getLastDetectionTime() > mBackgroundLScanStartTime) {
                        ...
                        if (System.currentTimeMillis() - mBackgroundLScanFirstDetectionTime
                            ...
                            Logger.d(TAG, "We've been detecting for a bit.  Stopping Android L background scanning");
                            try {
                                if (getScanner() != null) {
                                ...
                                Logger.w(TAG, "Cannot stop scan.  Bluetooth may be turned off.");
                            }
                            mBackgroundLScanStartTime = 0l;
                        }
                        else {
                            // report the results up the chain
                            Logger.d(TAG, "Delivering Android L background scanning results");
                            cycledLeScanCallback.onScanCycleEnd();
                        }
                    }
                }
            }
            Logger.d(TAG, "Waiting to start full bluetooth scan for another %s milliseconds",
                    millisecondsUntilStart);
            // Don't actually wait until the next scan time -- only wait up to 1 second.  this
            // allows us to start scanning sooner if a consumer enters the foreground and expects
            // results more quickly
            if (!mScanDeferredBefore && backgroundFlag) {
                setWakeUpAlarm();
            }
    ...
    protected void startScan() {
        ...
        if (backgroundFlag) {
            Logger.d(TAG, "starting scan in SCAN_MODE_LOW_POWER");
            settings = (new ScanSettings.Builder().setScanMode(ScanSettings.SCAN_MODE_LOW_POWER)).build();
        } else {
            Logger.d(TAG, "starting scan in SCAN_MODE_LOW_LATENCY");
    ...
    private BluetoothLeScanner getScanner() {
        if (mScanner == null) {
            Logger.d(TAG, "Making new Android L scanner");
            mScanner = getBluetoothAdapter().getBluetoothLeScanner();
            if (mScanner == null) {
                Logger.w(TAG, "Failed to make new Android L scanner");
            }
        }
        return mScanner;
    }
        ...
        if (leScanCallback == null) {
            leScanCallback = new ScanCallback() {
                ...
                public void onScanResult(int callbackType, ScanResult scanResult) {
                    ...
                    cycledLeScanCallback.onLeScan(scanResult.getDevice(),
                            scanResult.getRssi(), scanResult.getScanRecord().getBytes());
                    if (mBackgroundLScanStartTime > 0) {
                        Logger.d(TAG, "got a filtered scan result in the background.");
                    }
                }

                @Override
                public void onBatchScanResults(List<ScanResult> results) {
                    Logger.d(TAG, "got batch records");
                    for (ScanResult scanResult : results) {
                        cycledLeScanCallback.onLeScan(scanResult.getDevice(),
                                scanResult.getRssi(), scanResult.getScanRecord().getBytes());
                    }
                    if (mBackgroundLScanStartTime > 0) {
                        Logger.d(TAG, "got a filtered batch scan result in the background.");
                    }
                }
